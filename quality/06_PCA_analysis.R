...
write.table(genotype_num, file = "/shared/projects/multiwhaling/multiwhaling/data/VCF_filtered.geno", quote = F, 
            sep = "", row.names = F, col.names=F)

#### Faire la PCA
whale_pca <- pca(input.file = "/shared/projects/multiwhaling/multiwhaling/data/VCF_filtered.geno")
summary(whale_pca)
...
eigen_pca <- read.table("/shared/projects/multiwhaling/multiwhaling/data/VCF_filtered.pca/VCF_filtered.eigenvalues", header = F)
...
proj <- read.table("/shared/projects/multiwhaling/multiwhaling/data/VCF_filtered.pca/VCF_filtered.projections", header = F)
proj <- as_tibble(proj) |>
  dplyr::select(1:4) |>
  mutate(pop = names_ind |> arrange(Population))
saveRDS(proj, "/shared/projects/multiwhaling/multiwhaling/plot/PCA/projections.RDS")

pdf(paste("/shared/projects/multiwhaling/multiwhaling/plot/PCA/PCA_all_pop.pdf"))
proj |>
